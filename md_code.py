...
T2 = 1.5 * 1.380649*10**(-23)*300/(1.66*10**(-21))
K1 = 3/2*T1*(n-1)*kb
K2 = 3/2*T2*(n-1)*kb
...
def init_vel():
    vel = np.zeros((n, 3))
    sumV2_1 = 0.000005 #2*K1/m
    sumV2_2 = 0.0000002#2*K2/m # vx^2 + vy^2 + vz^2
    ...
    for atom in range(0, n):
        if atom == 0 or atom == 1 or atom == 2:
            vel[atom,0] = np.sqrt(sumV2_1)
        if atom == n-1 or atom == n-2 or atom == n-3:
            vel[atom,0] = -np.sqrt(sumV2_2)
        vel[atom, 0] -= mean_x
        vel[atom, 1] -= mean_y
        vel[atom, 2] -= mean_z
    return vel
...
particles = init_positions(x,y,z)
velocities = init_vel()
...
def Verlet(positions,velocities, f):
    ...
    for atom in range(0, n):
        ...
        velocities[atom][1] = v[1] + (delta/2) * (f_init[1]/mass)
        velocities[atom][2]= v[2] + (delta/2) * (f_init[2]/mass)
        # update position
        positions[atom][0] = p[0] + delta * v[0]
        positions[atom][1] = p[1] + delta * v[1]
        positions[atom][2] = p[2] + delta * v[2]
                 
    f_out, rFsum, potentials = calculateAllForcesAndPotentials(positions)
    for atom in range(0, n):
        velocities[atom][0] = v[0]+ (delta / 2) * (f_out[atom,0] / mass)
        velocities[atom][1] =  v[1] + (delta/2) * (f_out[atom,1]/mass)
        velocities[atom][2] = v[2]+ (delta/2) * (f_out[atom,2]/mass)
    return f_out,rFsum

########################################  This code runs the simulation - controls everything with time #####################################################

# Moves the particles in whatever direction they are forced
def performVerlet(positions,velocities,steps):
    all_forces = 0
    inst_Temp = []
    inst_Press = []
    all_kinetic = []
    ...
    for i in range(1,steps):
        ...
        F,rFsum = Verlet(positions,velocities,F)
        print("Force total", F)
        all_forces += F
        all_potentials += [U]
        all_kinetic += [K]
...
print("forcessss", F)
velocities = init_vel()
print("velocitiessss", velocities)
all_forces,all_potentials,all_kinetic,momentum,inst_Press,inst_Temp,avgU_lst = performVerlet(positions,velocities,steps)
